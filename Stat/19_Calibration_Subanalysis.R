...
model_type <- "Finetune" # Which results to use: V2, Finetune
REGULARIZE <- FALSE # Use Lasso regression
STRATIFY   <- NULL # Stratify using "AP" / "PA" / NULL
if(model_type == "V2") {
 which_training_set <- "d_train"; which_test_set <- "d_test_BWH" # d_test_MGH / d_test_BWH
...
for(i in 1) {  #7: DenseNet121v2 for V2; 1: DenseNet121v2 finetuned
 # Create dataset
 ...
 if(REGULARIZE) {
  ...
  pred_model_1  <- lapply(model_1, function(model) {as.numeric(predict(model, s = "lambda.1se",  newx = x_m1_test, type = "response"))})
  pred_model_2  <- lapply(model_2, function(model) {as.numeric(predict(model, s = "lambda.1se",  newx = cbind(x_m2_test, inter_m2_test), type = "response"))})
  pred_model_3  <- lapply(model_3, function(model) {as.numeric(predict(model, s = "lambda.1se",  newx = cbind(x_m3_test, inter_m3_test), type = "response"))})
  pred_model_DL <- lapply(model_DL, function(model) {as.numeric(predict(model, s = "lambda.1se",  newx = cbind(x_DL_test, inter_DL_test), type = "response"))})
 } else {
  ## Model specifications -----
  ...
  pred_model_1  <- lapply(model_1, function(model) {as.numeric(predict(model, s = 0,  newx = x_m1_test, type = "response"))})
  pred_model_2  <- lapply(model_2, function(model) {as.numeric(predict(model, s = 0,  newx = cbind(x_m2_test, inter_m2_test), type = "response"))})
  pred_model_3  <- lapply(model_3, function(model) {as.numeric(predict(model, s = 0,  newx = cbind(x_m3_test, inter_m3_test), type = "response"))})
  pred_model_DL <- lapply(model_DL, function(model) {as.numeric(predict(model, s = 0,  newx = cbind(x_DL_test, inter_DL_test), type = "response"))})
 }
...
PRED_test_MGH_1 <- pred_model_1; PRED_test_MGH_2 <- pred_model_2; PRED_test_MGH_3 <- pred_model_3; PRED_test_MGH_DL <- pred_model_DL
PRED_test_BWH_1 <- pred_model_1; PRED_test_BWH_2 <- pred_model_2; PRED_test_BWH_3 <- pred_model_3; PRED_test_BWH_DL <- pred_model_DL
PRED_test_BWHft_1 <- pred_model_1; PRED_test_BWHft_2 <- pred_model_2; PRED_test_BWHft_3 <- pred_model_3; PRED_test_BWHft_DL <- pred_model_DL
# =========
...
p_roc_test_MGH <- list()
for(i in 1) {
  calib <- caret::calibration(as.factor(abs(d_test_MGH$ANY_ENDPOINT_DEATH_30days-1)) ~ PRED_test_MGH_DL[[i]], cuts = 21)
 
  p_roc_test_MGH[[i]] <- ggplot(calib) + scale_fill_manual("Models: ", values = colors_4) + 
  xlab("Predicted Event Percentage [%]") +
  ylab("Observed Event Percentage [%]") +
  labs(title = "Internal test set (N=5,750)") +
  ...
  annotate("text", x = 100, y = 0, hjust = 1, vjust = 0,  size = 4,
           label = paste0("Integrated Calibration Index: ", format(round(gmish::ici(PRED_test_MGH_DL[[i]], abs(d_test_MGH$ANY_ENDPOINT_DEATH_30days-1)), 2), nsmall = 2)))
 #generalhoslem::logitgof(as.factor(abs(d_test_i[[roc_outcomes[i]]]-1)), PRED_test_MGH_DL[[i]], g = 21)
 }
...
p_roc_test_BWH <- list()
for(i in 1) {
 calib <- caret::calibration(as.factor(abs(d_test_BWH$ANY_ENDPOINT_DEATH_30days-1)) ~ PRED_test_BWH_DL[[i]], cuts = 21)
 
 p_roc_test_BWH[[i]] <- ggplot(calib) + scale_fill_manual("Models: ", values = colors_4) + 
  xlab("Predicted Event Percentage [%]") +
  ylab("Observed Event Percentage [%]") +
  labs(title = "External test set (N=22,764)") +
  theme_bw() + coord_fixed(ratio = 1) + #Theme
  theme(legend.position = "none",
        plot.title = element_text(size = 14),
        plot.subtitle = element_text(size = 8),
        ...
        axis.title = element_text(size = 12)) +
  annotate("text", x = 100, y = 0, hjust = 1, vjust = 0,  size = 4,
           label = paste0("Integrated Calibration Index: ", format(round(gmish::ici(PRED_test_BWH_DL[[i]], abs(d_test_BWH$ANY_ENDPOINT_DEATH_30days-1)), 2), nsmall = 2)))
}
...
for(i in 1) {
 calib <- caret::calibration(as.factor(abs(d_test_BWH_ft$ANY_ENDPOINT_DEATH_30days-1)) ~ PRED_test_BWHft_DL[[i]], cuts = 21)
 
 p_roc_test_BWHft[[i]] <- ggplot(calib) + scale_fill_manual("Models: ", values = colors_4) + 
  xlab("Predicted Event Percentage [%]") +
  ylab("Observed Event Percentage [%]") +
  labs(title = "External test set following fine tuning (N=4,552)") +
  theme_bw() + coord_fixed(ratio = 1) + #Theme
  theme(legend.position = "none",
        plot.title = element_text(size = 14),
        plot.subtitle = element_text(size = 8),
        ...
        axis.title = element_text(size = 12)) +
  annotate("text", x = 100, y = 0, hjust = 1, vjust = 0,  size = 4,
           label = paste0("Integrated Calibration Index: ", format(round(gmish::ici(PRED_test_BWHft_DL[[i]], abs(d_test_BWH_ft$ANY_ENDPOINT_DEATH_30days-1)), 2), nsmall = 2)))
}
# =========
...
fig_1 <- arrangeGrob(top = textGrob(paste0("Calibration plots of deep learning model to predict the composite endpoint"),
                                    gp = gpar(fontsize = 20, fontface = "bold", alpha = 1), vjust = 0.5),
                     p_roc_test_MGH[[1]], p_roc_test_BWH[[1]], p_roc_test_BWHft[[1]],
                     layout_matrix = layout)

ggsave(plot = fig_1, filename = paste0(folder_image, "Supp_Fig_3.pdf"), device = "pdf",
