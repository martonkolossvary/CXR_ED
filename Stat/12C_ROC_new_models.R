...
model_type <- "Finetune" # Which results to use: V2, Finetune, ALL
...
M2_99_thres <- max(which(roc_model_2[[1]]$sensitivities >= 0.99))
M2_99_thres <- roc_model_2[[1]]$thresholds[M2_99_thres]

M3_99_thres <- max(which(roc_model_3[[1]]$sensitivities >= 0.99))
M3_99_thres <- roc_model_3[[1]]$thresholds[M3_99_thres]

d_M2 <- ifelse(roc_model_2[[1]]$original.predictor < M2_99_thres, 0.4, 0.6)
d_M3 <- ifelse(roc_model_3[[1]]$original.predictor < M3_99_thres, 0.4, 0.6)

reclass <- reclassification(data = d_test_BWH_ft, cOutcome = which(colnames(d_test_BWH_ft) == label_col),
                            predrisk1 = d_M2, predrisk2 = d_M3,
                            cutoff = c(0, 0.5, 1))

table(d_test_BWH_ft$ANY_ENDPOINT_DEATH_30days, d_M2>0.5)
table(d_test_BWH_ft$ANY_ENDPOINT_DEATH_30days, d_M3>0.5)
